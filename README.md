...
The production stack is built for one machine: one Compose file, Caddy in front, Postgres in a
container, data in local volumes. A small VPS is enough. This section assumes Ubuntu 24.04 on
something like a Hetzner Cloud CX23 (2 vCPU, 4 GB, 40 GB disk), but nothing in it is specific to
one provider.

**Sizing.** Three JVMs, Postgres, Caddy, Prometheus and Grafana need a little over 2 GB at rest
(an estimate: read `docker stats` after the first start), and a deploy compiles the three
services on the server itself. 4 GB is the floor. The disk is small, so `deploy.sh` prunes
dangling images and week-old build cache after every successful run.

### Prepare the server, once

1. **Create it** with your SSH key added. In the provider's firewall allow 80 and 443 from
   anywhere and 22 from your own address only, and point the domain at it (step 1 above).

2. **Install Docker Engine and the Compose plugin** from
   [Docker's Ubuntu instructions](https://docs.docker.com/engine/install/ubuntu/), then create
   the account that will own the deployment:
   ...
   Membership of the `docker` group is root on that machine. That is why this account has no
   password, why its only key is the deploy key below, and why the key is restricted.

3. **Add swap**, as a safety net for the build (compiling three services next to a running
   stack is the one moment memory runs short):
...
4. **Clone and configure**, as `deploy`. The repository is public, so no credentials are needed:

   ```bash
   sudo mkdir -p /opt/century-road && sudo chown deploy: /opt/century-road
   git clone --branch main https://github.com/F3rren/century-road-backend.git /opt/century-road
   cd /opt/century-road && cp .env.example .env    # then fill it in, see "Setting up"
   ```

5. **Start it once by hand**, then do steps 5 and 6 of the production deployment above:
...
`.github/workflows/deploy.yml` deploys on its own when CI passes on a push to `main`, and can be
started by hand from the Actions tab (only from `main`). It connects to the server over SSH,
fast-forwards the checkout to the exact commit CI verified, and runs `infra/deploy/deploy.sh`.
That script rebuilds, waits until every healthcheck passes, and fails the run if one does not.
Nothing is built on GitHub, and no application secret passes through it: `.env` stays on the
server. Changing `.env` is therefore not a deploy. Edit it there and run
`docker compose -f compose-prod.yml up -d`.

Set it up once:

1. **Make a key just for this**, on your machine, and delete the private half once it is stored:

   ```bash
   ssh-keygen -t ed25519 -N '' -C github-deploy -f deploy_key
   ```

2. **Authorise it on the server**, in `/home/deploy/.ssh/authorized_keys`, with `restrict` in
   front so the key can run commands and nothing else (no forwarding, no terminal):

   ```
   restrict ssh-ed25519 AAAA... github-deploy
   ```

3. **Pin the server's identity.** From a session you already trust, on the server:

   ```bash
   echo "<server-ip> $(cut -d' ' -f1,2 /etc/ssh/ssh_host_ed25519_key.pub)"
   ```

   The workflow refuses any host that does not match this line, so a man in the middle is
   rejected rather than trusted on first use.

4. **In GitHub**, under *Settings → Environments*, create `production`. Restrict it to the `main`
   branch, and add yourself as a required reviewer if you want to approve each deploy. Then add,
   to that environment:

   | Kind | Name | Value |
   |---|---|---|
   | Secret | `DEPLOY_SSH_KEY` | the private key from step 1 |
   | Secret | `DEPLOY_KNOWN_HOSTS` | the line from step 3 |
   | Variable | `DEPLOY_HOST` | the server's IP address or hostname |
   | Variable | `DEPLOY_USER` | `deploy` |
   | Variable | `DEPLOY_PATH` | `/opt/century-road` |

There is no rollback button on purpose. Revert the commit on `main` through a pull request and
the same pipeline deploys the revert, so what is running is always something that is in git.

### Back up the database

Every user and password hash is in one Postgres volume on one disk, so a lost server is lost
data. `infra/backup/backup.sh` dumps the database, checks that the dump can be read back, and
deletes dumps older than 14 days (`BACKUP_KEEP_DAYS`). Files go to `~/backups/century-road`
(`BACKUP_DIR`), readable by `deploy` only. Run it nightly from `deploy`'s crontab (`crontab -e`):

```
0 3 * * * /opt/century-road/infra/backup/backup.sh >> "$HOME/century-road-backup.log" 2>&1
```

Restore a dump, replacing what is in the database. Only `auth-service` uses it, so only that one
is stopped:

```bash
cd /opt/century-road
docker compose -f compose-prod.yml stop auth-service
docker compose -f compose-prod.yml exec -T db sh -c \
  'pg_restore --clean --if-exists --no-owner -U "$POSTGRES_USER" -d "$POSTGRES_DB"' \
  < ~/backups/century-road/century-road-<timestamp>.dump
docker compose -f compose-prod.yml start auth-service
```

Two limits to know about. The dumps sit on the same disk as the database, so they protect against
a bad deploy or a deleted row but not against losing the server: copy them elsewhere too (your
provider's backup option for the whole machine, or `rclone` to object storage), because the
